...
export function CheckoutOrderSummary({ discount, items, onApplyPromo, onPromoCodeChange, onTermsChange, promoApplied, promoCode, processing, shipping, subtotal, tax, terms, total }: CheckoutOrderSummaryProps) {
    return (
        <section className="rounded-xl border border-stone-200 bg-white p-5 sm:p-6">
            <h2 className="text-sm font-bold text-stone-950 sm:text-base">Order Summary</h2>
                ...
                {items.map((item) => (
                    <div className="flex gap-3 py-3 first:pt-0 last:pb-0" key={item.id}>
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            <p className="mt-1 text-[10px] text-stone-500">Qty: {item.quantity}</p>
                        </div>
            ...
            <dl className="mt-5 space-y-3 border-t border-stone-100 pt-5 text-xs">
                <div className="flex justify-between gap-4 text-stone-500"><dt>Subtotal</dt><dd className="font-semibold text-stone-700">{currency.format(subtotal)}</dd></div>
                <div className="flex justify-between gap-4 text-stone-500"><dt>Express Delivery</dt><dd className="font-semibold text-stone-700">{shipping > 0 ? currency.format(shipping) : 'Free'}</dd></div>
                <div className="flex justify-between gap-4 text-stone-500"><dt>Tax (20% VAT)</dt><dd className="font-semibold text-stone-700">{currency.format(tax)}</dd></div>
                {discount > 0 && <div className="flex justify-between gap-4 text-emerald-600"><dt>Promo discount</dt><dd className="font-semibold">-{currency.format(discount)}</dd></div>}
            </dl>
            ...
            <div className="mt-5 flex gap-2">
                <div className="relative min-w-0 flex-1">
                    ...
                    <input className="min-h-10 w-full rounded-md border border-stone-200 bg-[#faf8f4] pl-9 pr-3 text-xs outline-none placeholder:text-stone-400 focus:border-[#bb915b]" placeholder="Promo code" type="text" value={promoCode} onChange={(event) => onPromoCodeChange(event.target.value)} />
                </div>
                <button className="text-[10px] font-semibold text-[#b38145] transition hover:text-stone-950" type="button" onClick={onApplyPromo}>Apply</button>
            </div>
            {promoApplied && <p className="mt-2 text-[10px] text-emerald-600">Promo code applied.</p>}
            ...
            <label className="mt-5 flex items-start gap-2 text-[10px] leading-4 text-stone-500">
                ...
                <span>I agree to the Terms &amp; Conditions and understand that payment processing is handled securely by our payment provider.</span>
            </label>
            <button className="mt-5 flex w-full items-center justify-center rounded-md bg-[#b88b52] px-4 py-3.5 text-sm font-semibold text-white transition hover:bg-[#a77e4c] disabled:cursor-not-allowed disabled:opacity-60" disabled={!terms || processing} type="submit">
                {processing ? 'Validating…' : 'Place Order'}
            </button>
            <p className="mt-2 flex items-center justify-center gap-1.5 text-[10px] text-stone-400"><LockKeyholeIcon aria-hidden="true" size={12} /> 256-bit SSL encryption</p>
        </section>
    );
}
