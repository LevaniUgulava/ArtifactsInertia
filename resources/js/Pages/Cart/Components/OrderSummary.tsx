...
export function OrderSummary({ discount, onApplyPromo, onPromoCodeChange, promoApplied, promoCode, shipping, subtotal, tax, total }: OrderSummaryProps) {
    ...
    return (
        <section className="rounded-xl bg-[#f8f5f0] p-5 sm:p-6">
            <h2 className="text-base font-bold text-stone-950">{t('orderSummary')}</h2>
            ...
            <div className="mt-6 border-t border-stone-200 pt-5">
                <label className="text-xs font-medium text-stone-700" htmlFor="promo-code">{t('promoCode')}</label>
                <div className="mt-2 flex gap-2">
                    ...
                    />
                    <button className="rounded-md bg-[#b88b52] px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#a77e4c]" type="button" onClick={onApplyPromo}>{t('apply')}</button>
                </div>
                {promoCode.length > 0 && !promoApplied && <p className="mt-2 text-[11px] text-stone-500">{t('promoHint')}</p>}
                {promoApplied && <p className="mt-2 text-[11px] text-emerald-600">{t('promoApplied')}</p>}
            </div>
