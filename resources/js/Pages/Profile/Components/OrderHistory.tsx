...
export function OrderHistory({ orders }: { orders: Order[] }) {
    ...
    return (
        <section>
            <SectionHeading
                action={<button className="inline-flex items-center gap-1 text-[11px] font-medium text-[#b38145] transition hover:text-stone-950" type="button">{t('viewAllOrders')} <ArrowRightIcon aria-hidden="true" size={12} /></button>}
                    ...
                    {orders.map((order) => (
                        <article className="flex flex-col gap-4 rounded-lg border border-stone-200 bg-white p-3 sm:flex-row sm:items-center sm:gap-5 sm:p-4" key={order.id}>
                            ...
                            <div className="min-w-0 flex-1">
                                <h3 className="text-xs font-semibold text-stone-900">{t('orderNumber', { number: order.id })}</h3>
                            ...
                            <button className="rounded-md border border-[#d7b37d] px-4 py-2 text-[10px] font-medium text-[#b38145] transition hover:bg-[#fbf5ec]" type="button">{t('details')}</button>
                        </article>
                    ))}
                </div>
            ) : (
                <p className="rounded-lg border border-dashed border-stone-200 px-5 py-8 text-center text-sm text-stone-500">{t('orderHistoryEmpty')}</p>
            )}
        </section>
    );
}
